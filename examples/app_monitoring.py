...
if __name__ == "__main__":
    ...
    device.setInput(SystemMonitoringInput())
    ...
    # Save to log file
    ep_data_file = FileEndpoint('./examples/logs/monitoring_log.txt')
    ep_data_file.setFormat(CSVFormat())
    device.addEndpoint(ep_data_file)

    ## Clear the dashboard
    # ep_clear = HTTPEndpoint(url='http://localhost:1880/clear_sysmon')
    # ep_clear.setCondition(EquationCondition('id == 1'))
    # device.addEndpoint(ep_clear)

    ## Send data to dashboard chart
    # ep_data = HTTPEndpoint(url='http://localhost:1880/datastream_sysmon')
    # ep_data.setFormat(JSONFormat())
    # device.addEndpoint(ep_data)

    ## Send messages to dashboard display
    # ep_upper = HTTPEndpoint(url='http://localhost:1880/msgstream_sysmon')
    # con_upper = ThresholdCondition(columns='cpu_pc', upper=10)
    # fmt_upper = MessageFormat('<font color=red>{timestamp} - CPU = {cpu_pc:.2f}%</font><br>')
    # ep_upper.setCondition(con_upper)
    # ep_upper.setFormat(fmt_upper)
    # device.addEndpoint(ep_upper)
    
    ## Send messages to dashboard display
    # ep_lower = HTTPEndpoint(url='http://localhost:1880/msgstream_sysmon')
